...
**Creating a project:** Open Settings → Agent Threads and choose **Projects** in the **Settings section** selector. Click **New project**, enter a project name and vault folder path, then **Create project**. By default, the working directory is derived as `<vault root>/<vault folder>`. Use the optional filesystem cwd override when the work belongs in a repo or directory outside the vault. Settings always shows the resolved effective cwd, and clearing the override returns the Project to its vault-derived path. You can also add a project context prompt — a few sentences describing the project's goals, conventions, and key files that Claude should always keep in mind.
...
**Managing projects:** Search or select a Project in the Projects settings section, edit its details, then choose **Save changes** or **Cancel**. Agents can make the same durable edits with `threads_update_project` (legacy alias: `obsidian_update_project`). It requires the target `projectId` and at least one changed field: an optional trimmed, nonblank `name`; a nullable `description`; or a nullable `cwdOverride`, which must be absolute when set. Passing `null` clears either nullable field. The tool returns the complete updated Project snapshot with its resolved `effectiveCwd`. Project members and Project Orchestrators may update their own Project; the Portfolio Orchestrator must provide a matching `elevatedProjectId`, and unassigned or cross-Project callers are denied.

Updating a Project does not relocate existing threads or restart their live sessions. New Project threads and dynamic Project schedules use the updated effective cwd, and newly initialized sessions receive updated Project context. This configuration tool does not automatically inject vault-relative/repo-relative path disambiguation; that remains a separate product follow-up. Create or open a Project Orchestrator from the detail pane; the first completed Project thread also creates one automatically without changing focus. Intentionally archiving the Project Orchestrator disables automatic recreation and completion wakeups, including after synced `data.json` is loaded on restart. Choosing **Create/Open** again deliberately re-enables it. Confirming Project deletion detaches its threads, clears pending proposed replies, removes the Project heartbeat, and pins its schedules to the former effective cwd.

...
| `ScheduleWakeup` | `delaySeconds`, `prompt`, `reason` | Schedules a message to be injected into this thread after a delay. Useful for polling CI, waiting for a deploy, or self-pacing a loop. While the wake-up is pending, the [Agent Dashboard](/docs/views/agent-dashboard/#waiting-threads) and [Kanban board](/docs/views/kanban-board/) keep the thread in **Waiting** with a live countdown (`Resumes in 4m — <reason>`). In the conversation, a compact [scheduled-activity pill](/docs/reference/status-line/#scheduled-activity) shows the countdown instead of a permanent banner; click it to inspect all wakeups and loops for the thread and cancel only the selected wakeup. Wakeups are persisted to disk (the same durable store backing [Scheduled Tasks](/docs/automation/scheduled-tasks/)), so a pending wakeup survives an Obsidian restart, plugin reload, or the machine sleeping — it fires on schedule, or immediately on next load if the fire time already passed. |

...
| `EnterDesignMode` | `brief` | Contributed by the separate **Design for Agent Threads** desktop plugin. Creates or reuses the calling thread's static design artifact, focuses that thread, and opens its preview and artifact controls. Returns artifact paths and design instructions so Claude or Codex can continue in the same turn. Requires a nonblank brief, a desktop filesystem vault, and write permission; unavailable during Plan mode or pending plan approval. See [Design artifacts](/docs/core-workflow/messaging-and-commands/#design-artifacts-in-geode). |
...
| `mcp_register_server` | `name`, `type`, `command?`, `args?`, `env?`, `url?`, `headers?` | Proposes a global external `stdio`, `http`, or `sse` MCP server and saves it only after a separate host confirmation. Registration is create-only: an identical retry is an unchanged no-op, while a different configuration under the same name is rejected. Credentials must use `${VAR_NAME}` placeholders and be collected with `request_secret`. It affects newly initialized sessions, never hot-loads the current session, and is unavailable to scheduled or noninteractive threads. See [Managing MCP Servers](/docs/integrations/mcp-servers/#registering-a-server-from-a-thread). |
| `watch_document` | `path` | Watches a vault note for content changes, owned by the calling thread. Any subsequent edit — from you, another thread, or a sync — sends this thread an injected alert message referencing the file via an `@[[filename]]` mention. Re-watching an already-watched path is a no-op that keeps the existing watch. See [Watch a document](/docs/core-workflow/messaging-and-commands/#watch-a-document). |

...
| `EnterDesignMode` | `brief` | Creates or reuses the calling thread's static design artifact, focuses that thread, and opens its preview and artifact controls. Returns artifact paths, the preview outcome, and design instructions so Claude or Codex can continue in the same turn. Requires a nonblank brief, a desktop filesystem vault, and write permission; unavailable during Plan mode or pending plan approval. See [Design artifacts](/docs/core-workflow/messaging-and-commands/#design-artifacts-in-geode). |

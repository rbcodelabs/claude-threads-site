...
`/model`, `/goal`, and `/loop` also work as prefixes in the Agents List and Agent Board dispatch boxes. The separate **Design for Agent Threads** plugin adds `/design` to those same surfaces:

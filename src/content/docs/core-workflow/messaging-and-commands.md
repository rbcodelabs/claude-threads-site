...
Starting in Agent Threads v0.43.0, Design is no longer built in. Download [Design v0.1.0](https://github.com/rbcodelabs/threads-design/releases/tag/v0.1.0) and follow its installation instructions. The Design repository is currently private and requires access; if you need Design but cannot access it, keep Agent Threads v0.42.0 until you can obtain the companion plugin. Enable Agent Threads first, then Design. Interactive preview and screenshot capture require Geode; Obsidian reveals the artifact source instead.
